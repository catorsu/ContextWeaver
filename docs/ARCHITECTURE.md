...
*   **Key Modules (Planned/Conceptual):**
    *   `contentScript.ts`: Injected into LLM web pages to handle UI triggers (e.g., `@` for general options, `@query` for real-time search). Manages DOM manipulation, creation, display, and logic of the floating UI panel (including dynamic rendering of search results, folder browsing views, and context selection options) and context block indicators. Includes the `showLoadingStateInPanel` function for displaying a standardized loading indicator. Communicates with the service worker for data fetching (workspace details, search, file/folder content, directory listings).
